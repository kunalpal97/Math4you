...
//         switchAnswer.push(allAnswers.splice(Math.ceil(Math.random() * (i+1)), 1)[0]);
...
function generate_equation() {
    let num1 = Math.floor(Math.random() * 8) + 5; // Ensuring a fair range (5-12)
    let num2 = Math.floor(Math.random() * 8) + 5; // Same range to keep it reasonable
    ...
    while (allAnswers.size < 4) {
        let dummyAnswer = answer + (Math.floor(Math.random() * 6) - 3); // Close values for difficulty
