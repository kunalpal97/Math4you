...
function generate_equation() {
    let num1 = Math.floor(Math.random() * 30) + 10; // Ensuring numbers stay within a fair range
    let num2 = Math.floor(Math.random() * 20) + 5;  // Making sure students get proper sums
    ...
    while (allAnswers.size < 4) {
        let dummyAnswer = answer + (Math.floor(Math.random() * 6) - 3); // Close values to real answer
        if (dummyAnswer >= 10 && dummyAnswer !== answer) {
